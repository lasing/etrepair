...
(function($) {
		...
		if (_f.tools) {
			if (typeof _f.tools == "string") {
				...
				for (var i = 0; i < _f.tools.length; i++) {
					...
					var _12 = $("<a href=\"javascript:void(0);\"></a>")
							.appendTo(td);
					_12[0].onclick = eval(_f.tools[i].handler || function() {
					});
					_12.linkbutton($.extend({}, _f.tools[i], {
						plain : true
					}));
				}
			}
	...
	function _13(_14, _15) {
		...
		if (_17.tabPosition == "left" || _17.tabPosition == "right") {
			...
			_18
					...
					.css("display", _17.showHeader ? "block" : "none");
			...
			ul._outerHeight(ul.outerHeight() - ul.height() - 1 + _17.tabHeight)
					.css("width", "");
			_19._size("height", isNaN(_17.height) ? "" : (_17.height - _18
					.outerHeight()));
		...
		if (_16.tabs.length) {
			...
			var _1b = _18.width()
					- _18.children(".tabs-tool:not(.tabs-tool-hidden)")
							._outerWidth();
			var _1c = Math.floor((_1b - d1 - d2 * _16.tabs.length)
					/ _16.tabs.length);
			$.map(_16.tabs, function(p) {
				_1d(p, (_17.justified && $.inArray(_17.tabPosition, [ "top",
						"bottom" ]) >= 0) ? _1c : undefined);
			});
			if (_17.justified
					&& $.inArray(_17.tabPosition, [ "top", "bottom" ]) >= 0) {
		...
		function _1d(p, _1f) {
			...
			var _1f = _1f ? _1f : (parseInt(_20.tabWidth || _17.tabWidth
					|| undefined));
			if (_1f) {
				p_t._outerWidth(_1f);
			} else {
		...
		$(_2f).children("div.tabs-header").unbind().bind("click", function(e) {
			if ($(e.target).hasClass("tabs-scroller-left")) {
				...
				function(e) {
					...
					if (li.length) {
						_31.onContextMenu.call(_2f, e, li.find(
								"span.tabs-title").html(), _32(li));
					}
				});
		function _32(li) {
			var _35 = 0;
	...
	function _36(_37) {
		...
		_39.removeClass("tabs-header-narrow").addClass(
				_38.narrow ? "tabs-header-narrow" : "");
		var _3b = _39.find(".tabs");
		_3b.removeClass("tabs-pill").addClass(_38.pill ? "tabs-pill" : "");
		_3b.removeClass("tabs-narrow")
				.addClass(_38.narrow ? "tabs-narrow" : "");
		_3b.removeClass("tabs-justified").addClass(
				_38.justified ? "tabs-justified" : "");
		if (_38.border == true) {
	...
	function _51(_52, _53) {
		...
		var _56 = _55.title;
		$.extend(_55, _53.options, {
			iconCls : (_53.options.icon ? _53.options.icon : undefined)
		});
		...
		if (_53.type == "all" || _53.type == "header") {
			...
			if (_55.header) {
				...
				if (_55.tools) {
					...
					if (!_59.length) {
						var _59 = $("<span class=\"tabs-p-tool\"></span>")
	...
	$.fn.tabs.methods = {
		...
		enableTab : function(jq, _8e) {
			return jq.each(function() {
				$(this).tabs("getTab", _8e).panel("options").tab
						.removeClass("tabs-disabled");
			});
		},
		disableTab : function(jq, _8f) {
			return jq.each(function() {
				$(this).tabs("getTab", _8f).panel("options").tab
	...
	$.fn.tabs.parseOptions = function(_94) {
		return $.extend({}, $.parser.parseOptions(_94, [ "tools",
