...
public class DaoAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails, UsernamePasswordAuthenticationToken authentication) throws AuthenticationException {
        ...
        logger.info("输入的密码是：" + presentedPassword);
        if (authentication.isEncrypted()) {
            logger.info("未加密的走法：" + presentedPassword);
            passwordEncoder = new PlaintextPasswordEncoder();// 不加密的处理 add by zhoupan 20160419
        } else {
            passwordEncoder = new Md5PasswordEncoder();
            logger.info("加密的走法：" + presentedPassword);
        }
    ...
    protected final UserDetails retrieveUser(String username, UsernamePasswordAuthenticationToken authentication) throws AuthenticationException {
        ...
        try {
            ...
            if (("没有为用户" + username + "指定角色").equals(notFound.getMessage())) {
                throw new BadCredentialsException(messages.getMessage("AbstractUserDetailsAuthenticationProvider.badCredentials", Constant.USER_NO_ROLES_ERROR));
            }
            throw notFound;
